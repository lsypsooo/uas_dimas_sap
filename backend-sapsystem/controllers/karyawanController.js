  ...
  try {
    ...
    if (!adminPerusahaanId) {
      return res.status(403).json({
        error: "Akses ditolak. Informasi perusahaan admin tidak ditemukan.",
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Gagal mengambil data karyawan", details: error.message });
  }
};
...
const createKaryawan = async (req, res) => {
  ...
  try {
    ...
    if (!adminPerusahaanId) {
      return res
        .status(403)
        .json({ error: "Akses ditolak. Informasi admin tidak valid." });
    }
    const hashedPassword = await bcrypt.hash(password, 10);
    const newKaryawan = await prisma.$transaction(async (tx) => {
      const existingUser = await tx.user.findFirst({
        where: { OR: [{ email }, { username: nama }] },
      });
      if (existingUser) {
        if (existingUser.email === email)
          throw new Error("Email sudah terdaftar");
        if (existingUser.username === nama)
    ...
    res
      .status(201)
      .json({ message: "Karyawan berhasil dibuat", data: newKaryawan });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ error: "Gagal membuat karyawan", details: error.message });
  }
};
...
const updateKaryawan = async (req, res) => {
  ...
  try {
    ...
    if (!adminPerusahaanId) {
      return res
        .status(403)
        .json({ error: "Akses ditolak. Anda bukan admin perusahaan." });
    }
    ...
    if (!karyawanToUpdate) {
      return res
        .status(404)
        .json({ error: "Karyawan tidak ditemukan di perusahaan Anda." });
    }
    ...
    if (email) {
      ...
      const emailExists = await prisma.user.findFirst({
        where: { email, NOT: { id: karyawanToUpdate.userId } },
      });
      if (emailExists)
    ...
    if (req.body.jatahCuti !== undefined)
      karyawanDataToUpdate.jatahCuti = parseInt(req.body.jatahCuti);
    ...
    res
      .status(500)
      .json({ error: "Gagal memperbarui karyawan", details: error.message });
  }
};
...
const deleteKaryawan = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Gagal menghapus karyawan", details: error.message });
  }
};
